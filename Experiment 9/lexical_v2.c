...
int main(int argc, char *argv[])
{
    ...
    char ch, word[200], kw[][10] = {"auto", "break", "case", "char", "const", "continue", "default", "do", "double", "else", "enum", "extern", "float", "for", "goto", "if", "int", "long", "register", "return", "short", "signed", "sizeof", "static", "struct", "switch", "typedef", "union", "unsigned", "void", "volatile", "while"};
    if(argc < 2)
    {
	...
	switch(state)
	{
	    case 0:
		    ch = fgetc(f);
		    if(isalnum(ch) || ch == '_')		//check for identifiers and integers
		    {
			...
			else
			{
			    ...
			    fseek(f, -1, SEEK_CUR);		//put back the look
			}
		    }
		    else if(ch == '&' || ch == '|')
		    {
			...
			else
			{
			    ...
			    fseek(f, -1, SEEK_CUR);		//put back the loop
			}
		    }
	    ...
	    case 4:
		    ...
		    if(isalpha(ch) || ch == '_')		//non digit or non punctuation charcater - error
		    {
			printf("\n Invalid Token\n");
			exit(1);
		    }
